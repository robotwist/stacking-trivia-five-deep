...
> **Superseded for playable content.** Use [`stacking-trivia-go-deep/docs/GOLD_STANDARD_STACK.md`](stacking-trivia-go-deep/docs/GOLD_STANDARD_STACK.md) and `npm run validate:stacks` instead. This file describes an older **35-question “ultimate”** model; the live app uses **exactly 5** narrative questions per stack. The Van Gogh examples below may disagree with the canonical chain (Anna Boch / *The Red Vineyard* / 400 francs).
